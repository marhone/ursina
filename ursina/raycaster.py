...
class Raycaster(Entity):
    ...
    def raycast(self, origin, direction=(0, 0, 1), distance=math.inf, traverse_target=scene, ignore=list(),
                debug=False):
    ...
    def boxcast(self, origin, direction=(0, 0, 1), distance=math.inf, thickness=(1, 1), traverse_target=scene,
        ...
        for y in range(3):
            for x in range(3):
                pos = origin + Vec3(lerp(-(thickness[0] / 2), thickness[0] / 2, x / (3 - 1)),
                                    lerp(-(thickness[1] / 2), thickness[1] / 2, y / (3 - 1)), 0)
